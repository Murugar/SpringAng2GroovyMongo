...
describe('MicropostNewComponent', () => {
  ...
  it('can create a post', () => {
    ...
    const inputEl2 = <HTMLInputElement>cmpDebugElement.query(By.css('input')).nativeElement;
    ...
    inputEl2.value = 'hello';
    
    spyOn(testCmp, 'doSomething');
    cmp.create(inputEl, inputEl2);
    expect(inputEl.value).toEqual('');
    expect(inputEl2.value).toEqual('');
    expect(testCmp.doSomething).toHaveBeenCalled();
  });

  it('does not create a post when content is blank', () => {
    const inputEl = <HTMLInputElement>cmpDebugElement.query(By.css('textarea')).nativeElement;
    const inputEl1 = <HTMLInputElement>cmpDebugElement.query(By.css('input')).nativeElement;
