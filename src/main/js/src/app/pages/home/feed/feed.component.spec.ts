...
describe('FeedComponent', () => {
  ...
  it('can show feed', () => {
    ...
    expect(cmp.feed.length).toEqual(3);

    const el = cmpDebugElement.nativeElement;
    expect(getDOM().querySelectorAll(el, 'li .content').length).toEqual(9);

    const el = cmpDebugElement.nativeElement;
    expect(getDOM().querySelectorAll(el, 'h3').length).toEqual(1);
    ...
    expect(content.innerHTML).toEqual('Title : title');
    
    const content1 = getDOM().querySelector(el, '.content.text-warning');
    
    console.log(content1); 

    expect(content1.innerHTML).toEqual('Content : content1');
